...
from lib.rtos_routine import task1, task2
...
class Host:
    ...
    def verify_commands(self, din):
        ...
        if "Tasks" in din:
            if din["Tasks"]["No"] == 1:
                self.dout["Task1"] = {}
                self.dout["Task1"]["values"] = []
                touch_pin = pin(int(din["Tasks"]["pins"][0]))
                self.touch = Touch_sensor(pin=touch_pin)
                task1(0, self.touch, self.dout)
                self.touch.deinit()  # Dont forget to deinit the pins

            if din["Tasks"]["No"] == 2:
                self.dout["Task2"] = {}
                self.dout["Task2"]["values"] = []
                ldr_pin = pin(int(din["Tasks"]["pins"][0]))
                self.ldr = LDR(pin=ldr_pin)
                task2(1, self.ldr, self.dout)
                self.ldr.deinit()

        if "DeviceSensor" in din:
